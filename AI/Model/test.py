...
input_image = Image.open("test1.jpg").resize((480, 288))
...
ort_session = onnxruntime.InferenceSession("nozcam_test_q.bin")
...
def detection_postprocess(image, cls_heads, box_heads):
    ...
    for cls_head, box_head in zip(cls_heads, box_heads):
        ...
        decoded.append(decode(torch.tensor(cls_head), torch.tensor(box_head), stride,
                              threshold=0.05, top_n=1000, anchors=anchors[stride]))
    
    decoded = [torch.cat(tensors, 1) for tensors in zip(*decoded)]
    scores, boxes, labels = nms(*decoded, nms=0.7, ndetections=3)
    return scores, boxes, labels

scores, boxes, labels = detection_postprocess(input_tensor, cls_heads, box_heads)
