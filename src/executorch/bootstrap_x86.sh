#!/usr/bin/env bash
# Build the x86-64 PiNozCam CPU daemon from a pinned ExecuTorch release.
set -euo pipefail
...
ET_BUILD_DIR="$ET_ROOT/cmake-out-x86_64"
...
if ! file "$daemon" | grep -q 'x86-64'; then
...
install -m 0755 "$daemon" \
  "$ARTIFACT_ROOT/bin/nozcam_daemon.x86_64.static"
sha256sum "$ARTIFACT_ROOT/bin/nozcam_daemon.x86_64.static"
