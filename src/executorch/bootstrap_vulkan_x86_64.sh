#!/usr/bin/env bash
# Build the x86-64 PiNozCam Vulkan daemon from pinned source.
set -euo pipefail
...
case "$(uname -m)" in
  x86_64|amd64) ;;
  *) echo "this must be a native x86-64 build (host is $(uname -m))" >&2; exit 1 ;;
esac
...
ET_BUILD_DIR="$ET_ROOT/cmake-out-vulkan-x86_64"
...
"$CMAKE_BIN" -G Ninja -S "$SHADERC_ROOT" -B "$SHADERC_BUILD_DIR" \
  -DCMAKE_BUILD_TYPE=Release \
  -DSHADERC_SKIP_TESTS=ON \
  -DSHADERC_SKIP_EXAMPLES=ON \
  -DSHADERC_SKIP_COPYRIGHT_CHECK=ON \
  -DSPIRV_SKIP_TESTS=ON
"$CMAKE_BIN" --build "$SHADERC_BUILD_DIR" --parallel "$JOBS" \
  --target glslc_exe
...
"$CMAKE_BIN" -G Ninja -S "$ET_ROOT" -B "$ET_BUILD_DIR" \
  -DCMAKE_BUILD_TYPE=Release \
...
if ! file "$daemon" | grep -Eq 'ELF 64-bit LSB.*x86-64'; then
  echo "daemon is not an x86-64 ELF" >&2
  file "$daemon" >&2
  exit 1
fi
...
install -m 0755 "$daemon" \
  "$ARTIFACT_ROOT/bin/nozcam_daemon.vulkan.x86_64"
file "$ARTIFACT_ROOT/bin/nozcam_daemon.vulkan.x86_64"
printf 'GNU_STACK=%s maximum-%s maximum-%s maximum-%s\n' \
  "$stack_flags" "$max_glibc" "$max_glibcxx" "$max_cxxabi"
sha256sum "$ARTIFACT_ROOT/bin/nozcam_daemon.vulkan.x86_64"
